...
// type Feature = GeoJSON.Feature<GeoJSON.Geometry, { name?: string; NAME_1?: string; province?: string }>;
...
//   useEffect(() => {
//     fetch("/geo/south-africa-provinces.json").then(r => r.json()).then(setFc);
//   }, []);
...
//   const getName = (f: Feature) => f.properties.name ?? f.properties.NAME_1 ?? f.properties.province ?? "Unknown";
...
//             key={i}
//             d={path(f as any) || ""}
//             onClick={() => { setActive(name); onSelect?.({ province: name }); }}
...
export default function SouthAfricaMap({
  ...
  const { path, projection } = useMemo(() => {
    ...
    const p = geoPath(proj);
    if (fc) {
      // Fit provinces into the SVG viewport
      // @ts-ignore d3-geo supports fitSize on projections
      proj.fitSize([width, height], fc);
    }
    return { path: p, projection: proj };
  }, [fc, width, height]);
